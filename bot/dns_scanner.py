...
class DnsScanner:
    ...
    def __init__(self):
        ...
        # Phase tracking ("idle", "masscan", "dns")
        self._phase = "idle"
        self._masscan_progress = ""  # e.g. "45.2% done, 1000 pps"
    ...
    def get_status(self) -> dict:
        ...
        return {
            ...
            "paused": self._paused,
            "phase": self._phase,
            "masscan_progress": self._masscan_progress,
            "domain": self._domain,
    ...
    def _masscan_pre_filter(self, cidr_text: str, port: int, rate: int) -> str:
        ...
        self._phase = "masscan"
        self._masscan_progress = "starting..."
        ...
        try:
            # Use Popen to stream stderr and parse progress in real-time
            proc = subprocess.Popen(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, text=True)
            for line in proc.stderr:
                line = line.strip()
                if not line:
                    continue
                # Parse masscan progress: "rate:  1000.00-kpps, 45.23% done"
                m = _re.search(r'(\d+\.\d+)% done', line)
                if m:
                    self._masscan_progress = f"{m.group(1)}% done"
                elif 'waiting' in line.lower():
                    self._masscan_progress = "waiting for replies..."
                if self._should_stop:
                    proc.terminate()
                    self._add_log("masscan stopped by user")
                    self._phase = "idle"
                    return ""
            proc.wait()
            ...
            self._add_log(f"masscan error: {e}")
            self._phase = "idle"
            return ""
            
        self._masscan_progress = "parsing results..."
        ...
        self._masscan_progress = f"{len(open_ips)} IPs found"
